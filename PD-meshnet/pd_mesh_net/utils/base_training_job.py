...
from TopGraph.model_graph import PersLoss
...
class BaseTrainingJob():
    ...
    def __init__(self,
        ...
        self.__use_gpu = True if torch.cuda.is_available() else False
        self.__compute_area_weighted_accuracy = compute_area_weighted_accuracy
        self.__verbose = verbose
        self.__use_pers = use_pers
    ...
    def _initialize_components(self,
        ...
        if (network_parameters is not None):
            ...
            # For now hard-coded. Shrec_16 has 30 classes
            self.__pers = None
            if self.__use_pers:
                self.__pers = PersLoss(input_dim=192,pers_im_size=100, pers_im_var=0.1, num_classes=30)

            # Move network to GPU if necessary.
            if (self.__use_gpu):
                self.__net.to("cuda")
                if self.__use_pers: 
                    self.__pers.to(torch.device('cuda'))

            else:
                self.__net.to("cpu")
                if self.__use_pers:
                    self.__pers.to(torch.device('cpu'))
        # Initialize dataset.
        ...
        if optimizer_parameters is not None:
            ...
            self.__opt_2 = None
            if self.__use_pers:
                self.__opt_2 = create_optimizer(network_parameters=self.__pers.parameters(), **optimizer_parameters)
        ...
        if (loss_parameters is not None):
            ...
            self.__loss_2 = None
            if self.__use_pers:
                self.__loss_2 = create_loss(**loss_parameters)
    ...
    def __save_checkpoint(self, batch_index=None):
        ...
        if self.__use_pers:
            checkpoint_dict['optimizer_2'] = self.__opt_2.state_dict()
            checkpoint_dict['persloss'] = self.__pers.state_dict()
    ...
    def train(self):
        ...
        while (self.__current_epoch <= self.__final_training_epoch):
            ...
            for data in self.__data_loader_train:
                ...
                if self.__use_pers:
                    self.__opt_2.zero_grad()
                outputs, log_info, embedding = self.__net(
                ...
                # targets_hat, f_val = self.__pers(embedding)
                if self.__use_pers:
                    targets_hat, f_val = self.__pers(embedding)
                    step_loss = 0.75 * self.__loss(outputs, targets) + 0.25 * self.__loss_2(targets_hat, targets)
                else:
                    step_loss = self.__loss(outputs, targets)
                # persloss = self.__loss_2(targets_hat, targets)
